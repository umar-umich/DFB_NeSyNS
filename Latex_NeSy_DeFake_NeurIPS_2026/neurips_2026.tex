...
% ------------------------------------------------------------------
\subsection{Evidential deep learning with gated evidence fusion}
\label{sec:edl}
% ------------------------------------------------------------------

We fuse the three evidence streams under the Evidential Deep Learning framework~\citep{sensoy2018evidential}, which places a Dirichlet prior over class probabilities to jointly model aleatoric and epistemic uncertainty.

\subsubsection{Gated evidence aggregation}

The total evidence is computed via gated addition:
\begin{equation}
  \bm{e}_\text{total} = \bm{e}_\text{spatial} + \sigma(\gamma_1)\,\bm{e}_\text{concept} + \sigma(\gamma_2)\,\bm{e}_\text{causal},
  \label{eq:gated_fusion}
\end{equation}
where $\gamma_1, \gamma_2 \in \mathbb{R}$ are learnable gate parameters.
The gates are initialized to $\gamma_1{=}{-}0.5$ and $\gamma_2{=}{-}0.8$ (sigmoid values $\approx 0.38$ and $0.31$, respectively), allowing the neuro-symbolic branches to contribute meaningful signal from the start of training while the spatial branch provides the dominant evidence.
As training progresses, the gates adapt: if the symbolic branches provide useful complementary signal, the gates open further; if not, they close.
This adaptive mechanism avoids the need to manually tune evidence weights and provides an interpretable measure of each branch's contribution.
...
\begin{equation}
  ...
  \label{eq:dirichlet}
\end{equation}
where $\bm{\alpha} \in \mathbb{R}^K_{>0}$ are the Dirichlet concentration parameters ($K{=}2$ for binary classification), $\hat{p}_k$ is the expected probability for class $k$, and $u \in (0, 1]$ is the epistemic uncertainty.
High total evidence ($S \gg K$) yields low uncertainty and confident predictions; low evidence yields $u \to 1$, signaling that the model lacks sufficient information.
...
The type-II maximum likelihood under the Dirichlet prior:
\begin{equation}
...
We apply per-sample class weights $[w_\text{real}{=}1.8,\, w_\text{fake}{=}0.6]$ to counteract the tendency of CLIP features to bias toward the fake class, ensuring balanced real/fake accuracy.

\paragraph{Annealed KL divergence.}
A KL penalty between the predicted Dirichlet (with correct-class evidence removed) and the uniform Dirichlet $\text{Dir}(\bm{1})$ regularizes evidence for incorrect classes:
\begin{equation}
  \mathcal{L}_\text{kl} = \text{KL}\bigl[\text{Dir}(\tilde{\bm{\alpha}}) \| \text{Dir}(\bm{1})\bigr], \quad
  \tilde{\alpha}_k = (1 - y_k)(\alpha_k - 1) + 1.
  \label{eq:kl}
\end{equation}
The KL weight is annealed from 0 to $\lambda_\text{kl}{=}0.15$ over the first 10 epochs to prevent premature evidence suppression.

\paragraph{Accuracy-vs-uncertainty calibration.}
Following~\citet{karandikar2021soft}, we add an AVU term that penalizes confident-but-wrong predictions and rewards uncertain-but-wrong predictions:
\begin{equation}
  ...
  \label{eq:avu}
\end{equation}
where $\hat{p}_\text{max} = \max_k \hat{p}_k$ is the predicted confidence.
This term directly calibrates the uncertainty estimate, ensuring that the model ``knows what it doesn't know.''

The total loss is:
\begin{equation}
  \mathcal{L} = \mathcal{L}_\text{nll} + \lambda_\text{kl}\,\beta(t)\,\mathcal{L}_\text{kl} + \lambda_\text{avu}\,\mathcal{L}_\text{avu},
  \label{eq:total_loss}
\end{equation}
where $\beta(t) = \min(1, t/T_\text{anneal})$ is the annealing schedule, $\lambda_\text{kl}{=}0.15$, and $\lambda_\text{avu}{=}0.1$.
...
\begin{table}[t]
  ...
  \begin{tabular}{llrl}
    ...
    \midrule
    \textbf{Fused evidence} & Gated Dirichlet aggregation & 2 & Evidential \\
    \bottomrule
  \end{tabular}
\end{table}
